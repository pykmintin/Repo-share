...
- [Reasoning](#reasoning) — Chain-of-thought · Rubric self-reflection · Plan-first · Simulate-before-execute
...
- [Recovery](#recovery) — Fresh-spawn · Check-revise-escalate · Continuation agents
...
- [Strategies to suspect](#strategies-to-suspect)
...
Forces real thinking rather than pattern-match. Default for most tasks is to skip reasoning; you ask for it explicitly, in a way the actor can't satisfy by performing reasoning-shaped output without content.

### Chain-of-thought (externalized reasoning)

**Mechanism.** Explicit intermediate steps turn a one-step inference into a multi-step sequence. Each step is itself a prediction; the context window holds partial calculations. Reduces arithmetic and logic errors measurably.

**As your own cognitive move.** Before committing to an answer on a multi-step problem, write out the steps. Don't skip "obvious" steps — the obviousness is often what hides the error. The act of writing forces the check.

**As worker-prompt language.**
> Work through this step by step. Name each step before executing it.

**Narrative case.** Asked to estimate token cost across three components, an agent answered "~50K total" — actually 87K. Forced to write out each component, the same agent caught the discrepancy in step 2.

**Avoid.** Lookup tasks where the answer is one fact. CoT bloats context without improving correctness.
...
**Mechanism.** Forces a plan artifact before execution. Plan must be concrete enough to execute against, requiring whole-task thinking before touching the work.

**As your own cognitive move.** When the task has multiple steps, write a numbered plan first. Each step: file touched, action taken, verification. Plan first, execute second. The plan often surfaces dependencies the eager-execute path would have missed.

**As worker-prompt language.**
> Before any implementation: produce a plan with numbered tasks. Each task must name the file touched, the action taken, and the verification command that proves it works. Plan first; implement second.

**Narrative case.** Without plan-first, an agent implementing a feature wrote step 1 → discovered step 1 was wrong because of an unforeseen dependency in step 3 → rewrote step 1 → discovered step 2 needed to change too. With plan-first, all three dependencies surfaced before any code was written.

**Slot:** 4, or as a standalone planner before an executor.

### Simulate-before-execute

**Mechanism.** Asks the actor to describe what the output will look like before producing it. CoT applied to output shape rather than reasoning steps. Surfaces format mismatches before they become wasted work.

**As your own cognitive move.** Before producing a file or a long output, describe its structure in one paragraph. "This file will have three sections: X, Y, Z. Section X will contain N rows of [shape]." Spot-check the description against the requirement before producing the artifact.

**As worker-prompt language.**
> Before writing the file: describe in one paragraph what it will contain and how it will be structured. Only after that, produce the file.

**Narrative case.** Without simulate-before-execute, a worker produced a 200-line markdown file in the wrong section structure — orchestrator caught it on review. With it, the worker described "I will produce sections A, B, C" — orchestrator caught the misread (it should have been A, B, D) before any line of the file existed.
...
### Check-revise-escalate with stall detection

**Mechanism.** Revision iterates with hard cap (3) and stall detector (issue count must decrease). Stall catches oscillation — actor fixing something each iteration but not converging.

**As your own cognitive move.** When you're iterating on your own draft, count the issues you're addressing. If the same issues persist across two iterations, you're stalling — escalate to the user rather than keep editing.

**As worker-prompt language (orchestrator logic).**
> After each revision: count blocker + warning issues. If `issues[n] < issues[n-1]`, continue. If not, or if n = 3, escalate to the user with current state and ask for direction.

**Narrative case.** Three rounds of revision on a worker output with no convergence — fix-A-breaks-B oscillation. Stall detector caught it after round 2; orchestrator escalated to user, who reframed the problem entirely.

**Slot:** Orchestrator behavior.

### Continuation agents (not resume)

**Mechanism.** When a worker pauses (checkpoint, too large, human input), it exits and a fresh worker continues. Fresh worker receives explicit state — completed tasks, current task, resume context — and verifies prior commits exist before acting.

**As your own cognitive move.** When you resume work after a pause (compaction, session break), don't trust your memory of state — reverify against artifacts. List completed tasks from disk, not from memory. Anything not on disk hasn't happened.

**As worker-prompt language.**
> You are a continuation agent. Previous worker completed tasks 1–3 (see `<completed_tasks>` below). Your first action: verify commits for tasks 1–3 exist via git log. If any missing, emit `## ISSUES FOUND` and stop. If all present, resume from task 4.

**Narrative case.** Continuation worker assumed previous tasks were committed — turned out only 2 of 3 had landed. Without verify-before-resume, worker added task 4 on top of incomplete state. With it, caught the gap immediately.

**Slot:** 1 + 2 + 4 for the continuation worker.
...
For your own thinking, the equivalent stack varies by use-case archetype. See `stacking.md`.

---

## Strategies to suspect

Not everything popular in the prompt-engineering corpus works. Empirically weak or null:

- "Take a deep breath" / "My job depends on this" / "I'll tip you $200." Community testing shows placebo.
- "You are an AI, but imagine you're not." Breaks alignment without providing a real lever; use specific role anchoring.
- Hidden-rubric self-reflection on binary tasks (overhead without benefit).
- Few-shot examples on verifiable-answer tasks (adds bias without value).

If a strategy doesn't have a mechanistic argument for why it would work, treat it as placebo until proven otherwise.
